    ...
    try:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                # Auth0 uses /oauth/token instead of /token. Retry once on 404 for compatibility.
                if token_response.status_code == 404:
                    auth0_token_url = f"{issuer_url}/oauth/token"
                    logger.warning(
                        "[callback] Token endpoint %s returned 404, retrying with %s",
                        token_url,
                        auth0_token_url,
                    )
                    token_response = await client.post(
