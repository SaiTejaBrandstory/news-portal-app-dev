...
async def validate_id_token(id_token: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        try:
            issuer_candidates = [settings.oidc_issuer_url]
            if settings.oidc_issuer_url.endswith("/"):
                issuer_candidates.append(settings.oidc_issuer_url.rstrip("/"))
            else:
                issuer_candidates.append(f"{settings.oidc_issuer_url}/")
            ...
            if payload is None and decode_error is not None:
                raise decode_error

            if payload is None:
                raise IDTokenValidationError("Token validation failed", "invalid_claims")
            # Log user hash instead of actual user ID to avoid exposing sensitive information
            user_id = payload.get("sub", "unknown")
            user_hash = hashlib.sha256(str(user_id).encode()).hexdigest()[:8] if user_id != "unknown" else "unknown"
...
def build_authorization_url(
    ...
    issuer_base = settings.oidc_issuer_url.rstrip("/")
    auth_url = f"{issuer_base}/authorize?" + urllib.parse.urlencode(params)
    return auth_url
...
def build_logout_url(id_token: Optional[str] = None) -> str:
    ...
    issuer_base = settings.oidc_issuer_url.rstrip("/")
    logout_url = f"{issuer_base}/logout?" + urllib.parse.urlencode(params)
    return logout_url
